...
export default function MatchDetail() {
    ...
    return (
        <main className="pt-32 px-6 min-h-screen relative z-10">
            <div className="max-w-3xl mx-auto">
                {/* Back Link */}
                <Link to="/matches" className="inline-flex items-center gap-2 text-white/60 hover:text-white mb-8 transition-colors">
                    ← Back to Matches
                </Link>

                {/* Status Banner */}
                {status === 'upcoming' && (
                    <div className="p-6 bg-blue-500/10 border border-blue-500/30 rounded-2xl text-center mb-8">
                        <p className="text-3xl font-black text-blue-400">⏳ Match Not Live Yet</p>
                ...
                {status === 'not_scheduled' && (
                    <div className="p-6 bg-white/5 border border-white/10 rounded-2xl text-center mb-8">
                        <p className="text-3xl font-black text-white/40">📅 Match Not Scheduled</p>
                        <p className="text-white/60 mt-2">This match has no scheduled date yet.</p>
                    </div>
                )}

                {status === 'live' && (
                    <div className="p-6 bg-green-500/10 border border-green-500/30 rounded-2xl text-center mb-8">
                        <p className="text-3xl font-black text-green-400 animate-pulse">🔴 LIVE</p>
                        {match.location && <p className="text-white/60 mt-2">📍 {match.location}</p>}
                    </div>
                )}

                {status === 'finished' && (
                    <div className="p-6 bg-white/5 border border-white/20 rounded-2xl text-center mb-8">
                        <p className="text-3xl font-black text-white/60">✓ Match Finished</p>
                        <p className="text-brand-secondary mt-2 font-bold">
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-8">
                    <div className="flex items-center justify-between gap-8">
                        ...
                        <div className="flex-1 text-center">
                            <p className={`text-2xl font-black mb-4 ${match.winner_id === match.team1?.id ? 'text-green-400' : 'text-white'}`}>
                            ...
                            {status === 'live' ? (
                                <div className="flex items-center justify-center gap-3">
                                    <button
                                        ...
                                        className="w-12 h-12 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg font-bold text-2xl"
                                        disabled={saving}
                                    >
                                        −
                                    </button>
                                    <span className="text-5xl font-black w-20 text-center">{scoreData.team1_score}</span>
                                    <button
                                        onClick={() => setScoreData({ ...scoreData, team1_score: scoreData.team1_score + 1 })}
                                        className="w-12 h-12 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg font-bold text-2xl"
                                        disabled={saving}
                                    >
                                        +
                                    </button>
                                ...
                                <p className="text-5xl font-black">{scoreData.team1_score}</p>
                            )}
                        </div>
                        ...
                        {/* Team 2 */}
                        <div className="flex-1 text-center">
                            <p className={`text-2xl font-black mb-4 ${match.winner_id === match.team2?.id ? 'text-green-400' : 'text-white'}`}>
                            ...
                            {status === 'live' ? (
                                <div className="flex items-center justify-center gap-3">
                                    ...
                                    <span className="text-5xl font-black w-20 text-center">{scoreData.team2_score}</span>
                                    <button
                                ...
                                <p className="text-5xl font-black">{scoreData.team2_score}</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                {status === 'live' && (
                    <div className="flex gap-4 mb-8">
                        <button
                            ...
                            disabled={saving}
                            className="flex-1 py-4 bg-brand-primary text-white font-bold rounded-xl hover:brightness-110 disabled:opacity-50 text-lg"
                        >
                        ...
                        <button
                            ...
                            disabled={saving}
                            className="flex-1 py-4 bg-green-500 text-white font-bold rounded-xl hover:brightness-110 disabled:opacity-50 text-lg"
                        >
